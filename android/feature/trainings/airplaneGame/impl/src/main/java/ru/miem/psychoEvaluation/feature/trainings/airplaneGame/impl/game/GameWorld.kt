...
class GameWorld (
    ...
    init {
        ...
        gameOverText = text(
            ...
            textSize = 18.0,
            font = AssetLoader.text
        ).xy(24, 55)  // TODO don't forget there are multiple directions of shadow

        highScoreShadow = text(
            text = "High Score!",
            textSize = 18.0,
            font = AssetLoader.shadow
        ).xy(19, 56)
        ...
        highScoreValuePrefixShadow = text(
            text = "High Score:",
            textSize = 18.0,
            font = AssetLoader.shadow
        ).xy(23, 106)

        highScoreValuePrefixText = text(
            text = "High Score:",
            textSize = 18.0,
            font = AssetLoader.text
        ).xy(22, 105)

        highScoreValueShadow = text(
            text = "",
            textSize = 18.0,
            font = AssetLoader.shadow
        )
    ...
    suspend fun update(delta: TimeSpan) {
        when (currentState) {
            GameState.READY -> {
            ...
            GameState.GAME_OVER -> {
                ...
                val scoreString = gameScoreProperty.value.toString()
    ...
    private suspend fun updateRunning(delta: TimeSpan) {
